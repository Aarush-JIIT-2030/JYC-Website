...
import './v21-ui-functional.css';
import './v18.19-jyc-restored.css';
import './v18.20-compact-theme.css';
import './v18.21-human-jyc.css';
...
function CustomCursor(){
 ...
 useEffect(()=>{
  ...
  const hover=e=>{
    ...
    const inOverlay=target?.closest?.('.search-overlay,.more-sheet-overlay,.assistant-overlay,.lightbox,.onboarding-overlay');
    const field=target?.closest?.('input,textarea,select,[contenteditable="true"]');
    const el=target?.closest?.('a,button,[data-cursor]');
    ring.current?.classList.toggle('is-hidden',!!inOverlay||!!field);
    dot.current?.classList.toggle('is-hidden',!!inOverlay||!!field);
    ring.current?.classList.toggle('is-hover',!!el&&!inOverlay&&!field);
    if(ring.current)ring.current.textContent=(el&&!inOverlay&&!field)?(el.getAttribute('data-cursor')||''):'';
  };
...
function Routes({data,admin,session,setAdmin,commit,notify}){
 ...
 const titles={
  ...
  '/planner':'JYC Planner | JIIT Youth Club',
 ...
 const descriptions={
  ...
  '/planner':'JYC Planner combines published JYC events and academic dates so students can plan around the campus calendar.',
 ...
 const knownPublic=['/','/about','/clubs','/events','/fests','/gallery','/team','/contact','/calendar','/planner','/map','/achievements','/resources','/guide'].includes(clean)&&clean!=='/recruitment'&&(clean!=='/fests'||isFestMode(data));
 const knownDetail=Boolean(club||event);
 const unknownRoute=!privateRoute&&!knownPublic&&!knownDetail;
 const pageType=club?'club':event?'event':clean==='/fests'?'fests':clean==='/clubs'?'clubs':clean==='/events'?'events':clean==='/gallery'?'gallery':clean==='/team'?'team':clean==='/resources'?'resources':clean==='/guide'?'guide':clean==='/map'?'map':clean==='/calendar'||clean==='/planner'?'calendar':clean==='/recruitment'?'recruitment':clean==='/about'?'about':clean==='/contact'?'contact':'home';
 ...
 if(clean==='/gallery')return <>{schema}<Gallery data={data}/></>;if(clean==='/team')return <>{schema}<Team data={data}/></>;if(clean==='/contact')return <>{schema}<Contact data={data}/></>;if(clean==='/recruitment')return <Navigate to="/" replace/>;if(clean==='/my-jyc')return <MyJYC data={data} session={session}/>;if(clean==='/calendar'||clean==='/planner')return <>{schema}<CalendarPage data={data}/></>;if(clean==='/notifications')return <Navigate to="/my-jyc" replace/>;if(clean==='/login')return <AccountLogin session={session}/>;if(clean==='/download')return <Navigate to="/guide" replace/>;if(clean==='/discover')return <Navigate to="/clubs" replace/>;if(clean==='/map')return <>{schema}<CampusMapPage data={data}/></>;if(clean.startsWith('/qr/'))return <QRSharePage data={data}/>;if(clean==='/moments')return <Navigate to="/gallery" replace/>;if(clean==='/agenda')return <Navigate to="/my-jyc" replace/>;if(clean==='/projects'||clean==='/projects/submit')return <Navigate to="/clubs" replace/>;if(clean==='/achievements'||clean==='/settings')return <Navigate to="/about" replace/>;if(clean==='/resources')return <Navigate to="/guide" replace/>;if(clean==='/guide')return <GuidePage data={data}/>;if(clean==='/admin')return <Admin data={data} admin={admin} setAdmin={setAdmin} commit={commit} notify={notify}/>;return <State title="Page not found." text="The page you requested does not exist."><Back/></State>
}
...
const explore=[
   ...
   ['/contact','Contact','Official JYC channels and email'],
   ['/my-jyc','My JYC','Saved clubs, events and registrations'],
   ['/planner','JYC Planner','Plan JYC events and academic dates'],
   ['/guide','JYC FAQ','Common questions, answered'],
   ['/map','Campus Map','Find JYC venues and campus locations']
  ];
 return <div ref={sheetRef} className="more-sheet-overlay" role="dialog" aria-modal="true" aria-label="More JYC" onMouseDown={e=>{if(e.target===e.currentTarget)close()}}>
  <div className="more-sheet jyc-more-club-sheet">
   ...
   <div className="more-sheet-head"><div><span className="eyebrow">MORE JYC</span><h3>Useful pages. Nothing repeated.</h3><p>About, contact, My JYC, Planner, FAQ and campus tools live here.</p></div><button className="sheet-close" onClick={close}>×</button></div>
