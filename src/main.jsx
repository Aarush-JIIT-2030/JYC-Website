...
import './v18.28-jyc-rebuild.css';
import './v19-jyc-complete.css';
import './v20-complete.css';
...
function Routes({data,admin,session,setAdmin,commit,notify,theme,setTheme}){
 ...
 const knownPublic=['/','/about','/clubs','/events','/fests','/gallery','/team','/contact','/archive','/calendar','/planner','/map','/achievements','/resources'].includes(clean)&&clean!=='/recruitment'&&(clean!=='/fests'||isFestMode(data));
 ...
 if(clean==='/gallery')return <>{schema}<Gallery data={data}/></>;if(clean==='/team')return <>{schema}<Team data={data}/></>;if(clean==='/contact')return <>{schema}<Contact data={data}/></>;if(clean==='/archive')return <ArchivePage data={data}/>;if(clean==='/recruitment')return <Navigate to="/" replace/>;if(clean==='/my-jyc')return <MyJYC data={data} session={session}/>;if(clean==='/calendar'||clean==='/planner')return <>{schema}<CalendarPage data={data}/></>;if(clean==='/notifications')return <Navigate to="/my-jyc" replace/>;if(clean==='/login')return <Navigate to="/my-jyc" replace/>;if(clean==='/download')return <Navigate to="/about" replace/>;if(clean==='/discover')return <Navigate to="/clubs" replace/>;if(clean==='/map')return <>{schema}<CampusMapPage data={data}/></>;if(clean.startsWith('/qr/'))return <QRSharePage data={data}/>;if(clean==='/moments')return <Navigate to="/gallery" replace/>;if(clean==='/agenda')return <Navigate to="/my-jyc" replace/>;if(clean==='/projects'||clean==='/projects/submit')return <Navigate to="/clubs" replace/>;if(clean==='/achievements'||clean==='/settings')return <Navigate to="/about" replace/>;if(clean==='/resources')return <Navigate to="/about" replace/>;if(clean==='/guide')return <Navigate to="/about" replace/>;if(clean==='/admin')return <Admin data={data} admin={admin} setAdmin={setAdmin} commit={commit} notify={notify} theme={theme} setTheme={setTheme}/>;return <section className="section page not-found-page"><div className="not-found-art"><span>404</span><img src={markLogo} alt="JYC Phoenix"/></div><span className="eyebrow">JYC · ROUTE MISSED</span><h1>The Phoenix missed this route.</h1><p>The page you requested is not part of the published JYC experience.</p><div className="detail-actions"><Button onClick={()=>nav('/')}>Return home</Button><Button secondary onClick={()=>nav('/clubs')}>Explore clubs</Button></div></section>
}
...
const explore=[
   ...
   ['/archive','JYC Archive','Browse the published JYC story by year']
  ];
 return <div ref={sheetRef} className="more-sheet-overlay" role="dialog" aria-modal="true" aria-label="More JYC" onMouseDown={e=>{if(e.target===e.currentTarget)close()}}>
  <div className="more-sheet jyc-more-club-sheet">
   ...
   <div className="more-sheet-head"><div><span className="eyebrow">MORE JYC</span><h3>Useful pages. Nothing repeated.</h3><p>About, contact, My JYC, Planner and campus tools live here.</p></div><button className="sheet-close" onClick={close}>×</button></div>
...
function Search({data,admin,close}){
 ...
 const pageIndex=[
  ...
  ['JYC Team','Meet the people behind JYC.','PAGE','/team'],['Contact JYC','Official JYC contact information.','PAGE','/contact'],['JYC Archive','Published JYC events, clubs, gallery moments and campus stories.','PAGE','/archive'],['My JYC','Your saved clubs and events.','PAGE','/my-jyc'],['JYC Calendar','Plan around published JYC events.','PAGE','/calendar'],
...
function Theme({theme,setTheme}){
 ...
 const flight=flying&&<div className={`jyc-flight-layer jyc-flight-${theme==='dark'?'to-light':'to-dark'}`} aria-hidden="true">
  <span className="jyc-flight-trace"/><span className="jyc-flight-spark spark-a"/><span className="jyc-flight-spark spark-b"/><span className="jyc-flight-spark spark-c"/>
...
function JYCAssistant({data,close}){
 ...
 const items=[
  ...
  {title:'JYC Planner',meta:'PLAN',text:'JYC events and academic dates.',path:'/planner',type:'PAGE'},
  {title:'My JYC',meta:'SYNC',text:'Saved clubs, events, registrations and personal agenda.',path:'/my-jyc',type:'PAGE'},
...
function HeroStats({communities=25,campuses=2,fests=2,since=2008}){
 ...
 const values=[useCountUp(communities,active),useCountUp(campuses,active),useCountUp(fests,active),useCountUp(since,active)];
 const items=[['Communities',values[0],'+'],['Campuses',values[1],''],['Flagship fests',values[2],'+'],['Since',values[3],'']];
...
function Fest({data}){
 ...
 const allowedFestThemes=['jyc','cyberpunk','diwali','neon','aurora','heritage'];
