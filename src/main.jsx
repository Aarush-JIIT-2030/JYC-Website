...
function GuidePage({data}){
 const nav=useNavigate();
 const steps=[
  ['01','Find a community','Browse official clubs by category and interest. You do not need to understand the whole ecosystem before starting.','/clubs'],
  ['02','Find what is happening','Use Events and the calendar to see workshops, talks, competitions, cultural experiences and other published JYC activities.','/events'],
  ['03','Show your work','Explore student projects or submit a build for review. A project can be technical, creative or community-focused.','/projects'],
  ['04','Use the campus layer','Open an event’s venue or the campus map when you need to understand where something is happening.','/map'],
  ['05','Make JYC yours','Save clubs and events, build your agenda and keep your own JYC trail in My JYC.','/my-jyc'],
 ];
 return <section className="section page guide-page">
  <div className="guide-hero reveal"><span className="eyebrow">JYC GUIDE · START HERE</span><h1>New to JYC?</h1><p>You do not need to know every club, event or campus space on day one. Start with one thing that interests you and follow the path.</p><div className="guide-actions"><button onClick={()=>nav('/clubs')}>Find a club →</button><button className="secondary" onClick={()=>nav('/events')}>See events →</button></div></div>
  <div className="guide-steps">{steps.map(([n,t,d,path])=><button className="guide-step reveal" key={n} onClick={()=>nav(path)}><span>{n}</span><div><strong>{t}</strong><p>{d}</p></div><b>→</b></button>)}</div>
...
function Routes({data,admin,session,setAdmin,commit,notify}){
 ...
 const descriptions={
  ...
  '/guide':'JYC Guide — the official route through JIIT Youth Club clubs, events, fests, recruitment, people and stories.',
...
function MobileMoreSheet({data,admin,close,openAssistant}){
 ...
 const explore=[
  ...
  ['/clubs','Clubs','Official JYC communities'],
  ['/events','Events','What JYC is doing next'],
  ['/team','Team','People behind JYC'],
  ['/gallery','Gallery','JYC moments and archive'],
  ['/calendar','Calendar','JYC events + academic dates'],
  ['/my-jyc','My JYC','Saved clubs and events'],
  ['/guide','JYC Guide','Understand the club'],
  ['/contact','Contact','Official JYC channels']
 ];
 return <div className="more-sheet-overlay" role="dialog" aria-modal="true" aria-label="More JYC" onMouseDown={e=>{if(e.target===e.currentTarget)close()}}>
  <div className="more-sheet jyc-more-club-sheet">
   ...
   <div className="more-sheet-section more-sheet-context"><span className="eyebrow">JYC CONTEXT</span><div className="more-context-card"><div><b>Communities → experiences → people → story.</b><p>Clubs are the heart of JYC. Events and fests are what the club creates. Team and Gallery show the people and memories behind it.</p></div><button onClick={()=>go('/about')}>Read the JYC story →</button></div></div>
...
function FirstVisitTour(){
 ...
 const steps=[
  ...
  ['04 · MORE','Find the rest of JYC.','Use More for About, Team, Gallery, Calendar, Contact, the JYC Guide and staff Control Center.','MORE'],
