...
import './v17-4-seo.css';
import './v18-3-final-ui.css';
...
import './v16-functional-ui.css';
import './v16-1-open-source-platform.css';
import './v16-2-iit-polish.css';
import './v16-3-platform.css';
import './v17-2-complete.css';
import './v18-4-product-overhaul.css';
import './v18-9-final.css';
import './v19-product-finish.css';
import './v19.7-jyc-club.css';
import './v19.8-contact-nav.css';
import './v18.9.9-deep-polish.css';
import './v19.10-runtime-ui.css';
import './v18.10-best-pass.css';
import './v18.11-final-product.css';
import './v18.12-best-jyc.css';
...
import {InteractivePhoenix,EcosystemSection,MomentsSection} from './v14-platform.jsx';
import {SkipLink,InstallPrompt,MaintenanceGate,ErrorBoundary,MyJYC,CalendarPage,EventTools,DownloadICS,RegistrationPage,AccountLogin,JsonLd,usePageMeta,EventReminderButton,Gallery,GalleryItems} from './extra-features.jsx';
...
const markLogo='/jyc-phoenix-reference-hd.png';
...
function CustomCursor(){
 ...
 useEffect(()=>{
  ...
  if(!supportsPointer||window.matchMedia('(prefers-reduced-motion: reduce)').matches)return;
  document.body.classList.add('jyc-custom-cursor');
  const move=e=>{
    const x=e.clientX,y=e.clientY;
    if(dot.current){dot.current.style.left=`${x}px`;dot.current.style.top=`${y}px`;dot.current.style.opacity='1'}
    if(ring.current){ring.current.style.left=`${x}px`;ring.current.style.top=`${y}px`;ring.current.style.opacity='1'}
  };
  ...
  window.addEventListener('pointerover',hover,true);
  window.addEventListener('pointerdown',down);
  window.addEventListener('pointerup',up);
  document.addEventListener('mouseleave',leave);
  return()=>{
    window.removeEventListener('pointermove',move);
    window.removeEventListener('pointerover',hover,true);
    window.removeEventListener('pointerdown',down);
    window.removeEventListener('pointerup',up);
    document.removeEventListener('mouseleave',leave);
    document.body.classList.remove('jyc-custom-cursor');
  };
 },[]);
 return createPortal(<><span ref={dot} className="jyc-cursor-dot"/><span ref={ring} className="jyc-cursor-ring"/></>,document.body);
}
function App(){const loc=useLocation();const nav=useNavigate();const [data,setData]=useState(empty);const [loading,setLoading]=useState(true);const [bootReady,setBootReady]=useState(false);const [bootStage,setBootStage]=useState('INITIALIZING');const [error,setError]=useState('');const [online,setOnline]=useState(()=>navigator.onLine!==false);const [theme,setTheme]=useState(()=>storageGet('jyc-theme')||(window.matchMedia?.('(prefers-color-scheme: dark)').matches?'dark':'light'));const [session,setSession]=useState(null);const [admin,setAdmin]=useState(null);const [toast,setToast]=useState(null);const [confirm,setConfirm]=useState(null);
 ...
 const content=<div className="app-frame"><SiteAtmosphere/><CustomCursor/><ScrollProgress/><BackToTop/><SkipLink/>{!isAdmin&&<><Navbar data={data} admin={admin} theme={theme} setTheme={setTheme}/><FirstVisitTour/></>} {connectionNotice}<div id="main-content" className={isAdmin?'app admin-app':'app'}><Routes data={data} admin={admin} session={session} setAdmin={setAdmin} commit={commit} notify={notify}/></div>{!isAdmin&&<Footer data={data} admin={admin}/>} {toast&&<div className={`toast toast-${toast.type||'success'}`} role="status"><span>{toast.type==='error'?'!':'✓'}</span>{toast.message}</div>} {confirm&&<ConfirmDialog request={confirm} onClose={ok=>{confirm.resolve?.(ok);setConfirm(null)}}/>}<InstallPrompt/></div>;
...
function ClubDetail({data,id}){
 ...
 return <section className={`section page club-detail club-theme-${clubTheme}`}><Breadcrumbs items={[{label:'Clubs',href:'/clubs'},{label:c.name}]}/>
  ...
  <div className="club-detail-layout">
   <main>
    ...
    {c.recruitment?.on&&<section id="club-join" className="club-section-card join-section reveal"><span className="eyebrow">JOIN THE CLUB</span><h2>There is a place for you here.</h2><div className="join-grid"><div className="join-card recruitment-card"><span className="tag">RECRUITMENT</span><h3>{c.recruitment.title||'Recruitment is open'}</h3>{c.recruitment.deadline&&<p>Deadline · {c.recruitment.deadline}</p>}{c.recruitment.link?<a className="btn" href={c.recruitment.link} target="_blank" rel="noopener noreferrer">Registration ↗</a>:<span className="muted-note">Application details will be published by the club.</span>}</div></div></section>}
   ...
   <aside className="club-side-panel reveal"><span className="eyebrow">FOLLOW</span><h3>{c.name}</h3><p>Follow this club to keep its events and opportunities close in My JYC.</p>{events.length>0&&<div className="club-next-event"><span>NEXT UP</span><strong>{events[0].title}</strong><small>{fmtDate(events[0].date)} · {events[0].start||'TBA'}</small></div>}{links.map(([n,u])=><a className="side-link" key={n} href={u} target="_blank" rel="noreferrer"><span>{n}</span><b>↗</b></a>)}<div className="side-divider"/><span className="eyebrow">JYC</span><p className="small-copy">JIIT Youth Club · Sector 128, Noida</p></aside>
  </div>
 </section>
}
function Team({data}){
 ...
 return <section className="section page team-page jyc-team-page unified-public-page">
  ...
  {members.length?orderedRoles.map(role=><section className="team-role-section reveal" key={role}><div className="team-role-head"><div><span className="eyebrow">JYC TEAM</span><h2>{role}</h2></div><span>{grouped[role].length} profile{grouped[role].length===1?'':'s'}</span></div><div className="team-grid team-grid-editorial">{grouped[role].map((m,i)=><article className="team-card team-card-editorial" key={m.id||`${role}-${i}`}><div className="team-photo team-photo-editorial">{m.photo||m.image?<img src={m.photo||m.image} alt={m.name||'JYC team member'} loading="lazy"/>:<img src={markLogo} alt="JYC Phoenix"/>}<span>{String(i+1).padStart(2,'0')}</span></div><div className="team-card-copy"><span className="tag">{m.role||m.position||'JYC Team'}</span><h3>{m.name||'JYC Team Member'}</h3>{(m.bio||m.description)&&<p>{m.bio||m.description}</p>}{m.clubId&&<small>{data.clubs.find(c=>String(c.id)===String(m.clubId))?.name||'JYC'}</small>}<div className="team-social-row">{m.linkedin&&<a href={m.linkedin} target="_blank" rel="noopener noreferrer">LinkedIn ↗</a>}{m.instagram&&<a href={m.instagram} target="_blank" rel="noopener noreferrer">Instagram ↗</a>}{m.email&&<a href={`mailto:${m.email}`}>Email ↗</a>}</div></div></article>)}</div></section>):<State title="The JYC team is being prepared." text="Official team profiles will appear here when JYC publishes them from the Control Center."><Button secondary onClick={()=>nav('/about')}>Learn about JYC →</Button></State>}
...
function GuidePage({data}){
 ...
 const faqs=[
  ['What is JIIT Youth Club (JYC)?','JYC is the apex student body for co-curricular activity at JIIT Noida, connecting student communities across technical, cultural, literary, sports, dramatics, photography and other interests.'],
  ['How do I find the right club for me?','Open Clubs from the navigation and filter by type, category or interest. Every club profile shows its people, links and current opportunities.'],
  ['How do I know what JYC is doing next?','Open Events for upcoming, live and past experiences, or use the Calendar to plan around published dates.'],
  ['How do I register for an event?','Open the event and use its registration action. Events with native registration let you sign up in-app; the rest link to official forms.'],
  ['Where is an event happening?','Use the Campus Map or the event’s own venue line to find where a session, workshop or fest stage is taking place.'],
  ['How do I keep my favourites together?','Save clubs and events into My JYC so registrations, saved spaces and your agenda live in one optional place.'],
  ['Where does the Gallery live?','Gallery keeps officially published JYC moments and albums together, reachable from the More menu.'],
  ['Can I contribute a project or open-source work?','JYC’s student ecosystem supports hands-on learning. Check the open-source community link below for JIIT developer programs.'],
  ['How do I contact the club?','The Contact page lists official JYC channels including Instagram, WhatsApp and email.']
 ];
 return <section className="section page guide-page jyc-guide-page unified-public-page"><div className="guide-hero reveal"><span className="eyebrow">JYC FAQ · START HERE</span><h1>Questions, answered.</h1><p>Straight answers about JIIT Youth Club clubs, events, fests, recruitment and campus life — with no redirect maze.</p></div>
  <div className="faq-list">{faqs.map((faq,i)=>{const [q,a]=faq;const isOpen=open===i;return <div className={`faq-item reveal ${isOpen?'open':''}`} key={i} style={{'--faq-delay':`${Math.min(i*40,320)}ms`}}><button className="faq-q" onClick={()=>setOpen(isOpen?null:i)} aria-expanded={isOpen}><span>{String(i+1).padStart(2,'0')}</span><strong>{q}</strong><b aria-hidden="true">{isOpen?'−':'+'}</b></button>{isOpen&&<div className="faq-a"><p>{a}</p></div>}</div>})}</div>
  <div className="guide-open-source reveal"><div><span className="eyebrow">OPEN-SOURCE MINDSET</span><h2>Learn by doing, then contribute.</h2><p>JIIT’s Open Source Developers’ Community currently describes a model built around hands-on workshops, hackathons, open repositories, starter issues and pathways into programs such as GSoC and Outreachy. JYC can apply that same beginner-friendly pattern to its wider student ecosystem without becoming an OSDC clone.</p></div><a href="https://platform.fossunited.org/c/jiit" target="_blank" rel="noopener noreferrer">See the JIIT open-source community ↗</a></div>
 </section>
}
...
function Routes({data,admin,session,setAdmin,commit,notify}){
 const nav=useNavigate();
 const p=useLocation().pathname;
 const clean=p.replace(/\/$/,'')||'/';
 const parts=clean.split('/').filter(Boolean);
 const slugId=parts[1]||'';
 const club=clean.startsWith('/clubs/')?data.clubs.find(c=>String(c.id)===String(slugId)||slug(c.name)===slug(slugId)):null;
 ...
 const titles={
  ...
  '/my-jyc':'My JYC | Saved Clubs, Events & Registrations',
 ...
 const descriptions={
  ...
  '/guide':'JYC FAQ — common questions about JIIT Youth Club clubs, events, fests, recruitment, My JYC and campus life.',
 ...
 const privateRoute=['/admin','/login','/my-jyc','/notifications','/settings','/agenda','/projects/submit','/download'].includes(clean)||clean.startsWith('/qr/')||clean.endsWith('/register');
 const knownPublic=['/','/about','/clubs','/events','/fests','/gallery','/team','/contact','/calendar','/map','/achievements','/resources','/guide'].includes(clean)&&clean!=='/recruitment'&&(clean!=='/fests'||isFestMode(data));
 const knownDetail=Boolean(club||event);
 const unknownRoute=!privateRoute&&!knownPublic&&!knownDetail;
 const pageType=club?'club':event?'event':clean==='/fests'?'fests':clean==='/clubs'?'clubs':clean==='/events'?'events':clean==='/gallery'?'gallery':clean==='/team'?'team':clean==='/resources'?'resources':clean==='/guide'?'guide':clean==='/map'?'map':clean==='/calendar'?'calendar':clean==='/recruitment'?'recruitment':clean==='/about'?'about':clean==='/contact'?'contact':'home';
 ...
 if(clean==='/about')return <>{schema}<About data={data}/></>;if(clean==='/clubs')return <>{schema}<Clubs data={data}/></>;if(club)return <>{schema}<ClubDetail data={data} id={slugId}/></>;
 ...
 if(clean==='/gallery')return <>{schema}<Gallery data={data}/></>;if(clean==='/team')return <>{schema}<Team data={data}/></>;if(clean==='/contact')return <>{schema}<Contact data={data}/></>;if(clean==='/recruitment')return <Navigate to="/" replace/>;if(clean==='/my-jyc')return <MyJYC data={data} session={session}/>;if(clean==='/calendar')return <>{schema}<CalendarPage data={data}/></>;if(clean==='/notifications')return <Navigate to="/my-jyc" replace/>;if(clean==='/login')return <AccountLogin session={session}/>;if(clean==='/download')return <Navigate to="/guide" replace/>;if(clean==='/discover')return <Navigate to="/clubs" replace/>;if(clean==='/map')return <>{schema}<CampusMapPage data={data}/></>;if(clean.startsWith('/qr/'))return <QRSharePage data={data}/>;if(clean==='/moments')return <Navigate to="/gallery" replace/>;if(clean==='/agenda')return <Navigate to="/my-jyc" replace/>;if(clean==='/projects'||clean==='/projects/submit')return <Navigate to="/clubs" replace/>;if(clean==='/achievements'||clean==='/settings')return <Navigate to="/about" replace/>;if(clean==='/resources')return <Navigate to="/guide" replace/>;if(clean==='/guide')return <GuidePage data={data}/>;if(clean==='/admin')return <Admin data={data} admin={admin} setAdmin={setAdmin} commit={commit} notify={notify}/>;return <State title="Page not found." text="The page you requested does not exist."><Back/></State>
}
...
function FirstVisitTour(){
 ...
 const steps=[
  ...
  ['03 · MY JYC','Keep your favourites close.','Save clubs and events you care about and keep them together in your optional My JYC space.','MY JYC','/my-jyc'],
  ['04 · MORE','Find the rest of JYC.','Use More for About, Gallery, Calendar, My JYC, the JYC FAQ and staff Control Center.','MORE'],
  ['READY','Ready to soar.','You now know the JYC route: Clubs → Events → My JYC. Start exploring the club.','DONE']
 ];
...
function Search({data,admin,close}){
 ...
 const pageIndex=[
  ['About JYC','About the JYC community and its purpose.','PAGE','/about'],['Clubs','Explore official JYC communities.','PAGE','/clubs'],['Events','Upcoming, live and past JYC experiences.','PAGE','/events'],
  ...(isFestMode(data)?[['Fests','Flagship JIIT and JYC fest experiences.','PAGE','/fests']]:[]),
  ['JYC Team','Meet the people behind JYC.','PAGE','/team'],['Contact JYC','Official JYC contact information.','PAGE','/contact'],['My JYC','Your saved clubs and events.','PAGE','/my-jyc'],['JYC Calendar','Plan around published JYC events.','PAGE','/calendar'],
 ...
 const adminPages=[['Control Center','Staff publishing, reviews, registrations and operations.','ADMIN','/admin']];
 const rawResults=[
  ...adminPages.map(x=>({key:'a'+x[3]+x[0],type:x[2],title:x[0],meta:x[1],searchText:`${x[0]} ${x[1]}`,icon:'⚙',go:()=>nav(x[3])})),
  ...pageIndex.map(x=>({key:'p'+x[3],type:x[2],title:x[0],meta:x[1],searchText:`${x[0]} ${x[1]}`,icon:'⌘',go:()=>nav(x[3])})),
 ...
 const intentResults=term?(
  ...
  /(my registrations|my agenda|saved|my jyc)/.test(term)?[{key:'intent-my',type:'QUICK ACTION',title:'My JYC',meta:'Open your saved spaces, registrations and agenda',searchText:'my jyc saved registrations agenda',icon:'◎',priority:120,go:()=>nav('/my-jyc')}]:
  []
 ):[];
 ...
 const content=<div className="overlay search-overlay" role="dialog" aria-modal="true" aria-label="JYC search" onMouseDown={close} onTouchStart={e=>{if(e.target===e.currentTarget)close()}}>
  <div className="search-modal" onMouseDown={e=>e.stopPropagation()}>
   ...
   {!q&&<div className="search-suggestions"><span>EXPLORE</span>{[['EXPLORE CLUBS','/clubs'],['FIND EVENTS','/events'],...(isFestMode(data)?[['JIIT FESTS','/fests']]:[]),['JYC TEAM','/team'],['MY JYC','/my-jyc']].map(([label,path])=><button key={path} onClick={()=>{remember(label.toLowerCase());nav(path);close()}}>{label}<b>↗</b></button>)}</div>}
...
function Loading({stage='INITIALIZING',cached=false}){
 ...
 return <div className="loading" role="status" aria-live="polite" aria-label="Loading JYC"><div className="loading-grid"/><div className="loading-flight loading-flight-a"/><div className="loading-flight loading-flight-b"/><div className="loading-orbit loading-orbit-a"/><div className="loading-orbit loading-orbit-b"/><div className="loading-center"><div className="loading-kicker"><i/> JIIT · SECTOR 128 · NOIDA</div><div className="loading-scene"><span className="loading-ghost loading-ghost-a"><img src="/jyc-phoenix-reference-hd.png" alt=""/></span><span className="loading-ghost loading-ghost-b"><img src="/jyc-phoenix-reference-hd.png" alt=""/></span><span className="loading-halo"/><span className="loading-mark"><img src="/jyc-phoenix-reference-hd.png" alt="JYC Phoenix"/></span><span className="loading-flight-dot dot-a"/><span className="loading-flight-dot dot-b"/><span className="loading-flight-dot dot-c"/></div><div className="loading-logo"><span>JIIT YOUTH CLUB</span><strong>READY TO SOAR</strong></div><div className="loadline" style={{'--boot-progress':`${progress}%`}}><i/></div><div className="loading-status"><span>{current[0]} · {progress}%</span><span>{current[1]}</span></div><div className="loading-steps" aria-hidden="true"><i className={progress>=18?'done':''}/><i className={progress>=44?'done':''}/><i className={progress>=72?'done':''}/><i className={progress>=100?'done':''}/></div></div></div>
}
...
function JYCAssistant({data,close}){
 ...
 const items=[
  ...
  {title:'Calendar',meta:'PLAN',text:'JYC events and academic dates.',path:'/calendar',type:'PAGE'},
  {title:'Contact',meta:'CONNECT',text:'Official JYC contact channels.',path:'/contact',type:'PAGE'},
  ...data.clubs.filter(c=>c.published&&c.status!=='archived').slice(0,80).map(c=>({title:c.name,meta:`CLUB · ${c.category||c.type||'COMMUNITY'}`,text:c.description||'',path:`/clubs/${slug(c.name)}`,type:'CLUB'})),
  ...data.events.filter(e=>e.published&&!e.archived).slice(0,80).map(e=>({title:e.title,meta:`EVENT · ${e.club||'JYC'}`,text:`${e.venue||''} ${e.description||''}`,path:`/events/${slug(e.title)}`,type:'EVENT'}))
 ];
 const results=q.trim()?rankSearchResults(items,q).filter(x=>x._score>0).slice(0,8):items.slice(0,6);
 return <div className="assistant-overlay" role="dialog" aria-modal="true" aria-label="JYC Assistant" onMouseDown={e=>{if(e.target===e.currentTarget)close()}}><div className="assistant-panel-modern"><div className="assistant-head"><div><span className="eyebrow">JYC ASSISTANT</span><h2>Where do you want to go?</h2><p>Navigation help for clubs, events, people, gallery, calendar and contact.</p></div><button className="sheet-close" onClick={close} aria-label="Close JYC Assistant">×</button></div><label className="assistant-search"><span>⌕</span><input autoFocus value={q} onChange={e=>setQ(e.target.value)} onKeyDown={e=>{if(e.key==='Escape')close()}} placeholder="Search JYC…" aria-label="Search JYC Assistant"/></label><div className="assistant-results" role="listbox">{results.map((x,i)=><button key={`${x.path}-${i}`} role="option" onClick={()=>{close();nav(x.path)}}><span>{String(i+1).padStart(2,'0')}</span><div><strong>{x.title}</strong><small>{x.meta}</small></div><b>→</b></button>)}{q&&results.length===0&&<div className="assistant-empty">No JYC destination matches “{q}”. Try a club or event name.</div>}</div><div className="assistant-shortcuts">{['Clubs','Events','Team','Calendar'].map(x=><button key={x} onClick={()=>{close();nav('/'+x.toLowerCase().replace('calendar','calendar'))}}>{x}</button>)}</div></div></div>
}
...
function Home({data}){
 ...
 return <div className="home"><section className="hero"><div className="hero-copy"><span className="eyebrow">{h.heroEyebrow||'JIIT · SECTOR 128 · NOIDA'}</span><h1>{h.heroTitle||'JIIT YOUTH CLUB'}</h1><div className="hero-rule"/><h4>{h.heroSubtitle||'READY TO SOAR'}</h4><p>{h.heroLead||''}</p><HeroStats clubs={clubs.length} team={data.team.filter(m=>m.published===true).length} upcoming={upcoming.length} moments={data.gallery.length}/><div className="hero-context-rail" aria-label="JYC status"><span><i/>JYC · SECTOR 128</span><span>STUDENT LED</span><span>{data.events.some(e=>e.published&&!e.archived&&eventState(e)==='live')?'LIVE NOW':'READY TO SOAR'}</span></div><div className="hero-quick"><button onClick={()=>nav('/about')}><span>01</span><strong>{h.quick1Title||'About JYC'}</strong><em>{h.quick1Text||'Know the community →'}</em></button><button onClick={()=>nav('/contact')}><span>02</span><strong>{h.quick3Title||'Connect'}</strong><em>{h.quick3Text||'Find JYC online →'}</em></button></div></div><div className="hero-art" ref={heroArtRef}><div className="sky-stars" aria-hidden="true"/><InteractivePhoenix sceneUrl={h.phoenixSceneUrl}/>{nextEvent&&<div className="next-event-plaque"><small>NEXT JYC MOMENT</small><strong>{nextEvent.title}</strong><span>{fmtDate(nextEvent.date)} · {nextEvent.venue||'Venue TBA'}</span></div>}<div className="reference-side-note" aria-hidden="true"><span>STUDENT LED</span><span>COMMUNITY DRIVEN</span><span>IMPACT FOCUSED</span></div><span className="float-chip chip1">STUDENT CULTURE</span><span className="reference-campus-badge">128 · NOIDA</span><span className="float-chip chip2">READY TO SOAR</span></div></section>{data.announcement.on&&data.announcement.text&&<div className="announcement reveal"><span>JYC UPDATE</span><p>{data.announcement.text}</p>{data.announcement.link&&<a href={data.announcement.link} target="_blank" rel="noopener noreferrer">Open ↗</a>}</div>}<JYCPulse data={data}/><NextMoment data={data}/><ImpactStats data={data}/><EcosystemSection data={data}/><div className="home-layout"><MomentsSection data={data}/>{visibleLayout.map(k=>render[k])}</div></div>
}
function About({data}){return <section className="section page unified-public-page about-page"><Breadcrumbs items={[{label:'About JYC'}]}/><Back label="Back to home" to="/"/><SectionHead eyebrow="ABOUT JYC" title={data.homepage.aboutTitle||'The JYC story belongs here.'} text={data.homepage.aboutText||'Official JYC information will appear here once supplied and approved by JYC.'}/><div className="about-layout"><div className="quote-card"><img src={logo}/><span>READY TO SOAR</span></div><div><h3>What is JYC?</h3><p className="large-copy">JIIT Youth Club (JYC) is the student-led campus body that connects JIIT clubs, student communities, events, fests and co-curricular activities.</p><h3>Purpose</h3><p className="large-copy">{data.homepage.aboutText||'This page is structured for the official JYC description, vision, mission and activities. Content can be updated from the Control Center without changing the design.'}</p><div className="number-list"><div><b>01</b><span>Holistic student development</span></div><div><b>02</b><span>Communities and campus experiences</span></div><div><b>03</b><span>Creativity, technology and participation</span></div></div></div></div></section>}
...
function ClubCard({c,index=0}){
 ...
 return <Card className="club-card club-card-premium tilt-card reveal" onClick={()=>nav('/clubs/'+slug(c.name))}><div className="club-card-visual" style={c.banner?{backgroundImage:`linear-gradient(135deg,var(--bg-card),transparent 65%),url(${c.banner})`}:{}}><div className="club-logo">{c.logo?<img src={c.logo} alt={`${c.name} logo`} loading="lazy"/>:<img src={logo} alt="JIIT Youth Club logo"/>}</div><span className="club-index">{String(index+1).padStart(2,'0')}</span><button className={`save-btn ${saved?'saved':''}`} onClick={toggle} aria-label={saved?'Unfollow club':'Follow club'}>{saved?'Following':'Follow club'}</button></div><div className="card-body"><div className="card-topline"><span className="tag">{c.type} · {c.category||'Community'}</span>{c.pinned&&<span className="pin-badge">PINNED</span>}</div><h3>{c.name}</h3><p>{c.description||'Official JYC club information will appear here.'}</p><div className="chips">{(c.interests||[]).slice(0,4).map(i=><span key={i}>{i}</span>)}</div><div className="club-card-footer"><span className="text-link">Explore club</span><span>↗</span></div></div></Card>
}
...
function EventCard({e,index=0}){
 ...
 return <Card className="event-card event-card-premium tilt-card reveal" onClick={()=>nav('/events/'+slug(e.title))}><div className="event-poster" style={e.poster?{backgroundImage:`url(${e.poster})`}:{}}><span className={state==='live'?'live':''}>{state==='live'?'LIVE NOW':fmtDate(e.date)}</span><span className="event-number">{String(index+1).padStart(2,'0')}</span><div className="event-card-tools"><button className={`save-btn ${saved?'saved':''}`} onClick={toggle}>{saved?'Saved':'Save event'}</button></div></div><div className="card-body"><div className="card-topline"><span className="tag">{e.club||'JYC'}</span>{e.pinned&&<span className="pin-badge">PINNED</span>}</div><h3>{e.title}</h3><p>{e.description||'Event details and registration information.'}</p><div className="event-card-meta"><span><b>{e.start||'TBA'}</b>{e.end?` – ${e.end}`:''}</span><span>{e.venue||'Venue TBA'}</span></div><span className="text-link">View event details →</span></div></Card>
}
function EventDetail({data,id,session}){
 const e=findEntity(data.events,id);
 const [saved,setSaved]=useState(()=>e?storageGet(`jyc-saved-event-${e.id}`)==='1':false);
 useEffect(()=>{setSaved(e?storageGet(`jyc-saved-event-${e.id}`)==='1':false)},[e?.id]);
