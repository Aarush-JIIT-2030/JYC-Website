...
import {InteractivePhoenix} from './v14-platform.jsx';
import {JYCTicker,JYCNowStrip,JYCClubDirectory,JYCEventTimeline,JYCMomentsRail,JYCArchive,JYCActivityFeed} from './v18.18-motion.jsx';
...
const explore=[
 ...
 return <div ref={sheetRef} className="more-sheet-overlay" role="dialog" aria-modal="true" aria-label="More JYC" onMouseDown={e=>{if(e.target===e.currentTarget)close()}}>
  <div className="more-sheet jyc-more-club-sheet">
   ...
   <div className="more-sheet-head"><div><span className="eyebrow">MORE JYC</span><h3>Useful pages. Nothing repeated.</h3><p>About, contact, My JYC, FAQ and campus tools live here.</p></div><button className="sheet-close" onClick={close}>×</button></div>
...
function Home({data}){
 ...
 const live=upcoming.filter(e=>eventState(e)==='live');
 const clubs=data.clubs.filter(c=>c.published&&c.status!=='archived');
 const team=data.team.filter(m=>m.published===true);
 return <div className="home j18-home">
  <section className="j18-hero">
   <div className="j18-hero-copy">
    <div className="j18-hero-kicker"><span className="j18-live-dot"/>{h.heroEyebrow||'JIIT · SECTOR 128 · NOIDA'}</div>
    <h1><span>JIIT</span><span>YOUTH</span><span className="j18-hero-accent">CLUB</span></h1>
    <p>{h.heroLead||'The student-led body behind JIIT clubs, events, fests and the people who make campus life happen.'}</p>
    <div className="j18-hero-actions"><button className="j18-primary" onClick={()=>nav('/clubs')}>Find a club <span>↗</span></button><button className="j18-secondary" onClick={()=>nav('/events')}>See what's next <span>→</span></button></div>
    <div className="j18-hero-meta"><span>{clubs.length||'—'} published clubs</span><i/><span>{upcoming.length||'—'} upcoming moments</span><i/><span>{team.length||'—'} team profiles</span></div>
   </div>
   <div className="j18-hero-art"><div className="j18-hero-art-label">JYC / 128</div><div className="j18-phoenix-wrap"><InteractivePhoenix sceneUrl={h.phoenixSceneUrl}/></div>{nextEvent&&<button className="j18-next-event" onClick={()=>nav(`/events/${slug(nextEvent.title)}`)}><span>{live.length?'LIVE NOW':'NEXT UP'}</span><strong>{nextEvent.title}</strong><small>{fmtDate(nextEvent.date)} · {nextEvent.venue||'Venue TBA'} <b>↗</b></small></button>}</div>
  </section>
  <JYCTicker/>
  <JYCNowStrip data={data}/>
  <main className="j18-home-flow">
   <JYCClubDirectory data={data} limit={8}/>
   <JYCEventTimeline data={data} limit={6}/>
   <JYCMomentsRail data={data} limit={8}/>
   <JYCActivityFeed data={data}/>
   <JYCArchive data={data}/>
   <section className="j18-join reveal"><div><span className="eyebrow">JYC · OPEN CAMPUS</span><h2>{h.ctaTitle||'There is more to JIIT than the timetable.'}</h2><p>{h.ctaText||'Find a community, turn up to an event, or just see what people on campus are building.'}</p></div><div className="j18-join-actions"><button onClick={()=>nav('/clubs')}>Explore clubs ↗</button><button onClick={()=>nav('/about')}>What is JYC? →</button></div></section>
  </main>
 </div>
}
...
function Clubs({data}){
 ...
 const list=visible.filter(c=>type==='All'||c.type===type).filter(c=>cat==='All'||c.category===cat).filter(c=>interest==='All'||(c.interests||[]).includes(interest)).filter(c=>!q||`${c.name} ${c.category} ${(c.interests||[]).join(' ')} ${c.description||''}`.toLowerCase().includes(q.toLowerCase()));
 return <section className="section page clubs-page unified-public-page j18-clubs-page"><Breadcrumbs items={[{label:'Clubs'}]}/><div className="j18-page-hero reveal"><div><span className="eyebrow">JYC CLUBS</span><h1>Find your people.</h1><p>Browse the published JIIT communities by type, category and interest. Hover a club on desktop to preview it; tap to open it.</p></div><div className="j18-page-count"><strong>{visible.length}</strong><span>published clubs</span></div></div>
  <div className="j18-club-filters reveal"><div className="j18-filter-tabs" aria-label="Club type">{['All','Technical','Non-Technical'].map(x=><button className={type===x?'active':''} onClick={()=>{setType(x);setCat('All');setInterest('All')}} key={x}>{x==='All'?'Everything':x}</button>)}</div><label className="j18-search-field"><span>⌕</span><input value={q} onChange={e=>setQ(e.target.value)} placeholder="Search clubs, categories or interests" aria-label="Search clubs"/>{q&&<button type="button" onClick={()=>setQ('')} aria-label="Clear club search">×</button>}</label><select value={cat} onChange={e=>setCat(e.target.value)} aria-label="Club category"><option value="All">All categories</option>{cats.map(x=><option key={x}>{x}</option>)}</select><select value={interest} onChange={e=>setInterest(e.target.value)} aria-label="Club interest"><option value="All">All interests</option>{ints.map(x=><option key={x}>{x}</option>)}</select></div>
  <div className="j18-club-page-list reveal">{list.length?<div className="j18-club-page-rows">{list.map((c,i)=><ClubCard key={c.id} c={c} index={i}/>)}</div>:<State title="No matching clubs." text="Try another club name, category or interest."/>}</div>
 </section>
}
